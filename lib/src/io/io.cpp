...
std::string get_absolute_material_path(const std::string & mat_file, const std::string & obj_file)
{
    if (mat_file[0] == '/')
    {
        ...
        std::cout << abs_mat_file << std::endl;
        return abs_mat_file;
    }
}
...
std::shared_ptr<Material> tobj_to_mh_material(const tinyobj::material_t & tobj_material, const std::string & obj_filename)
{
    ...
    {
        mh_material->setDiffuseTexture(std::make_shared<Texture>(get_absolute_material_path(tobj_material.diffuse_texname, obj_filename)));
    }

    if (tobj_material.specular_texname != "")
    {
        mh_material->setSpecularTexture(std::make_shared<Texture>(get_absolute_material_path(tobj_material.specular_texname, obj_filename)));
    }
...
std::vector<std::shared_ptr<Mesh> > loadMeshesFromOBJ(const std::string & path)
{
    ...
    for (size_t shape_idx = 0; shape_idx < shapes.size(); ++shape_idx) {
        ...
        for (size_t face_idx = 0; face_idx < shapes[shape_idx].mesh.num_face_vertices.size(); ++face_idx) {
            tinyobj::index_t vert_0_idx = shapes[shape_idx].mesh.indices[index_offset + 0];
            tinyobj::index_t vert_1_idx = shapes[shape_idx].mesh.indices[index_offset + 1];
            tinyobj::index_t vert_2_idx = shapes[shape_idx].mesh.indices[index_offset + 2];
        ...
        // convert material
        size_t material_id = shapes[shape_idx].mesh.material_ids[0] >= 0 ? shapes[shape_idx].mesh.material_ids[0] : 0;
        std::shared_ptr<Material> mesh_material = tobj_to_mh_material(materials[material_id], path);
